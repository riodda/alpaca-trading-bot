...
def alpaca_submit_market_buy_order(_symbol,_qty,_time_in_force,_client_order_id='WATB'+(datetime.now().strftime('_%Y-%m-%dT%H:%M:%S.%f'))):    
...
def alpaca_submit_market_notional_buy_order(_symbol,_qty,_time_in_force,_client_order_id='WATB'+(datetime.now().strftime('_%Y-%m-%dT%H:%M:%S.%f'))):      
    _order = api.submit_order(symbol=_symbol,notional=_qty,side='buy',type='market',time_in_force=_time_in_force,client_order_id=_client_order_id)
    return _order

def alpaca_submit_limit_buy_order(_symbol,_qty,_limit_price,_time_in_force,_client_order_id='WATB'+(datetime.now().strftime('_%Y-%m-%dT%H:%M:%S.%f'))):     
